...
client.on("message", async message => {
  ...
  case "play":
	if(message.channel.type == 'text') {
	  ...
	  if(stuff[0].toLowerCase().includes("jontv.me")) {
		  jontv(stuff[1], function(videodata) {
			  try {
  ...
  case "np":
	  ...
	  if(guilds[message.guild.id]) {
		  ...
		  jontv(guilds[message.guild.id].queueID[0], function(videodata) {
			  try {
				  let JSONobj = JSON.parse(videodata);
				  jontv.getuser(JSONobj.cid, function(userdata){
					  if(userdata) {
...
guilds[message.guild.id].dispatcher = connection.playArbitraryInput(`http://JonTV.me/${encodeURI(guilds[message.guild.id].queueF[0])}`);
